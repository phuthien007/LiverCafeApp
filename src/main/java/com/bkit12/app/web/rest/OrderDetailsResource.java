...
/**
 * REST controller for managing {@link com.bkit12.app.domain.OrderDetails}.
 */
@RestController
@RequestMapping("/api")
public class OrderDetailsResource {
    ...
    private static final String ENTITY_NAME = "cafeAppOrderDetails";
    ...
    public ResponseEntity<OrderDetailsDTO> updateOrderDetails(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, orderDetailsDTO.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<OrderDetailsDTO> partialUpdateOrderDetails(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, orderDetailsDTO.getId().toString())
        );
    }
    ...
    public ResponseEntity<List<OrderDetailsDTO>> getAllOrderDetails(
        ...
        Page<OrderDetailsDTO> page;
        if (eagerload) {
            page = orderDetailsService.findAllWithEagerRelationships(pageable);
        } else {
            page = orderDetailsService.findAll(pageable);
        }
